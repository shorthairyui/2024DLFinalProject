...
class VisionTransformer(nn.Module):
    ...
    def custom_attn(self, attn_layer, x, ex_feats=None, beta=1.2, gamma=3.0, token_size=(16, 16)):
        ...
        #print(q.shape)
        #kk = k.transpose(1, 2).reshape(1, 442, -1)[:, 1:, :]
        #qq = q.transpose(1, 2).reshape(1, 442, -1)[:, 1:, :]
        #vv = v.transpose(1, 2).reshape(1, 442, -1)[:, 1:, :]
        #torch.save({'q': qq, 'k': kk, 'v': vv}, 'clipqkv.pt')
        #print("v0",v.shape)
        v = v.contiguous().view(-1, bsz * num_heads, head_dim).transpose(0, 1)

        B, C, H, W = ex_feats.shape   #1,768,21,21
        #在这里去掉归一化##不完全正确，归一化包括下面的similarity = (similarity - torch.mean(similarity) * beta) * gamma
        q_k = F.normalize(ex_feats.flatten(2, 3), dim=1) #归一化
        #print("qk",q_k.shape)
        similarity = torch.einsum("b c m, b c n -> b m n", q_k, q_k)#x*xT

        similarity = (similarity - torch.mean(similarity) * beta) * gamma
        similarity[similarity < 0.0] = float('-inf')  #这里也是mask
        mask = similarity.to(q.dtype).unsqueeze(1).repeat(1, num_heads, 1, 1)#maskingprint
        ...
        v = F.interpolate(v, size=(H, W), mode='bilinear', align_corners=False)#插值
        v = v.permute(0, 2, 3, 1).reshape(bsz*num_heads, H*W, head_dim)

        attn_output = torch.bmm(attn_weights, v)
        attn_output = attn_output.transpose(0, 1).contiguous().view(-1, bsz, embed_dim)
        attn_output = attn_layer.out_proj(attn_output)#投影
        return attn_output
        '''
        num_heads = attn_layer.num_heads
        _, bsz, embed_dim = x.size()
